...
- 🎯 **多站点支持**: AnyRouter、Linux.do、OpenI、GitHub、ShareYourCC
- 🍪 **Cookie 持久化**: 自动保存和加载，支持快速登录
- 🔧 **统一 CLI**: 简洁的命令行界面
- 📦 **清晰架构**: 代码、配置、数据完全分离
- 🛡️ **完善日志**: 每站点独立日志，错误自动截图
- 🤖 **自动化任务**: 支持签到、抽奖等每日任务自动化
...
```json
{
  ...
  "users": [
    ...
    {"site": "github", "username": "用户名", "password": "密码"},
    {"site": "shareyourcc", "email": "邮箱@example.com", "password": "密码"}
  ]
}
```
...
### GitHub
- **登录方式**: 账号密码表单登录 + 2FA 支持
- **凭据配置**: `config/users.json` - `users` 数组（`site: "github"`）
- **特性**:
  - Cookie 快速登录
  - 自动处理登录表单（支持邮箱/用户名）
  - 2FA 手动输入支持（60秒超时）
...
### ShareYourCC
- **登录方式**: LinuxDO OAuth 授权 或 邮箱密码登录
- **凭据配置**: `config/users.json` - `users` 数组（`site: "shareyourcc"`）
- **特性**:
  - **智能登录**: 优先 Cookie → LinuxDO OAuth → 邮箱密码
  - **自动化任务**: 登录后自动执行每日签到和抽奖
  - **弹窗处理**: 自动检测并关闭系统公告等弹窗（19个选择器）
  - **卡片式 UI 适配**: 支持现代卡片式和传统按钮式布局
  - **签到功能**: 12个选择器，自动滚动，智能遍历可见元素
  - **抽奖功能**: 直接导航到抽奖页面，点击开始抽奖，等待结果
  - **OAuth 复用**: 自动复用 LinuxDO 登录状态，避免重复登录
  - **页面跳转处理**: 自动处理 OAuth 和抽奖的页面跳转
- **技术亮点**:
  - 登录后自动导航到 `/dashboard/lottery` 进行抽奖
  - 等待3秒查看抽奖结果后自动返回仪表板
  - 详细的日志记录，包括元素匹配数量和索引
  - 完善的异常处理，单个元素失败不影响其他尝试
